class VendedorsController < ApplicationController
  ...
  def index
    ...
    @vendedors = Vendedor.find_by_sql("SELECT v.*, u.* FROM  vendedors v, usuarios u WHERE v.usuario_id = u.id");
    @usuarios = Usuario.where(tipo: "v")
  end

  # GET /vendedors/1
  # GET /vendedors/1.json
  def show
    @usuario = Usuario.find(@vendedor.usuario_id)
    ...
    cadSQL = "SELECT u.nombre_usuario, u.apellidos, u.celular, v.fecha_venta FROM Kali_Base.usuarios u, Kali_Base.venta v WHERE v.usuario_id = u.id AND v.vendedor_id = "+(@vendedor.usuario_id.to_s) +" order by fecha_venta DESC";
  ...
  private
    ...
    def vendedor_params
      params.require(:vendedor).permit(:codigo, :usuario_id)
    end
